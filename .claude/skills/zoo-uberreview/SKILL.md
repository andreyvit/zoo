...
Ask reviewers to use `zoo-refactoring` for findings that affect pre-existing code outside the current task: consequential cross-cutting gaps should become proposals, broad mundane refactors should become separate subtasks/commits, and small low-pollution fixes may stay in the current task.
